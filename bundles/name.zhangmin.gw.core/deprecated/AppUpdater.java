...
public class AppUpdater extends AbstractEventSubscriber {
	 ...
	 */
	@Override
	public void receiveState(String appName, Data newData) {
		if (appRegistry != null) {
			try {
				GenericApp app = (GenericApp) appRegistry.getApp(appName);
				boolean isAccepted = false;
				if (app.getAcceptedStateTypes().contains(newData.getClass())) isAccepted = true;
				else {
					for (Class<? extends State> data : app.getAcceptedStateTypes()) {
						try {
