	 ...
	 private final class ThingHandlerTracker extends ServiceTracker<ThingHandler, ThingHandler> {
	    ...
	    private ThingListener thingListener = new ThingListener() {
	        ...
	        public void channelUpdated(ChannelUID channelUID, State state) {
	            String app = appChannelLinkRegistry.getBoundApp(channelUID);
	            if (app != null) {
	                eventPublisher.postState(app, state);
	            }
	        }
	    };
	    ...
	    @Override
	    public void receiveState(String appName, State newState) {
	        for (Thing thing : this.things) {
	            List<Channel> channels = thing.getChannels();
	            for (Channel channel : channels) {
	                if (isLinked(appName, channel)) {
	                    ...
	                    if (handler != null) {
	                        logger.info(
	                                ...
	                                newState, appName, channel.getUID());
	                        try {
	    ...
	    public void setAppChannelLinkRegistry(AppChannelLinkRegistry appChannelLinkRegistry) {
	    ...
	    public void unsetAppChannelLinkRegistry(AppChannelLinkRegistry appChannelLinkRegistry) {
